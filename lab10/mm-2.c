 ...
 *     2. use explicit linked free list and 9 size classes, 52 (just?)
...
static void *extend_heap(size_t size) 
{
    ...
    /* coalesce free blocks and return */
    return coalesce(bp);
}
...
static void add_to_flist(void *bp, size_t size)
{
    ...
    /* note: if the next free block is the tail block,
       need not modify the pointers */
    bp = NEXT_FBLKP(bp);
    if (!IS_EMPTY_BLKP(bp)) 
        SET_PREV_PTR(bp, GET(flist_headp));
}
