...
#
# 1. implement the iaddq instruction.
#
# 2. change prediction strategy and reduce bubbles: in the main loop,
#    jxx instructions for counting positive items first take the next 
#    pc to add to the summary, only when the jump condition is 
#    true, bubbles are set and the current pc is set to the 
#    next instruction. In such condition, the bubble of Decode stage
#    can be removed since the current instruction in Decode stage is 
#    just the correct instruction to be executed.
...
wordsig D_valC 'if_id_curr->valc'
...
word f_pc = [
	...
	M_icode == IJXX && W_icode == IOPQ && D_icode != IRMMOVQ && M_Cnd : M_valA;
	M_icode == IJXX && W_icode != IOPQ && !M_Cnd : M_valA;
...
word f_predPC = [
	...
	f_icode == IJXX && D_icode != IOPQ : f_valC;
	1 : f_valP;
];
...
word d_valA = [
	D_icode == IJXX && E_icode == IOPQ : D_valC;
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && ((M_icode == IOPQ && f_icode != IRMMOVQ && e_Cnd) || (M_icode != IOPQ && !e_Cnd))) ||
	# Stalling at fetch while ret passes through pipeline
...
bool E_bubble =
	# Mispredicted branch
	(E_icode == IJXX && ((M_icode == IOPQ && e_Cnd) || (M_icode != IOPQ && !e_Cnd))) ||
	# Conditions for a load/use hazard
	E_icode in { IMRMOVQ, IPOPQ } &&
	 E_dstM in { d_srcA, d_srcB};
