...
def expect_req(proc, req, log, timeout=-1):
    if proc != None:
...
def do_part1_4(server, server_port, client, proxy, in_part1_7=False):
    ...
    # crash in response
    send_req(client, req)
    expect_req(server, req, 'part1_4_1')
    ...
    send_req(client, req)
    expect_req(server, req, 'part1_4_2')
    ...
    send_req(client, req)
    expect_req(server, req, 'part1_4_3')
...
def do_part1_6(server, server_port, client, proxy, in_part1_7=False):
    ...
    send_req(client, req)
    expect_req(server, req, 'part1_6_1')
    ...
    send_req(client, req)
    expect_req(server, req, 'part1_6_2')
    resp = HttpResponse(random_status(), long_resp)
    send_resp(server, resp)
    expect_resp(client, resp)
    expect_log(proxy, client.ip, proxy_uri(server_ip, server_port, long_path), len(resp.raw))
...
def do_part2_1(server, server_port, client, proxy):
    ...
    for i in xrange(PART2_1_ROUND):
        ...
        expect_req(server, bigreq, 'part2_1_1')
        ...
        for j in xrange(len(cut_point) / 2):
            ...
            expect_req(server2, smallreq, 'part2_1_2')
...
class ConcurrentPayload(object):
    ...
    def do(self):
        ...
        if not self.req_done:
            ...
            if work_bytes == len(self.req.raw) - self.req_pos:
                expect_req(self.server, self.req, 'part2_1_3')
...
def main():
    ...
    fail_tests = []
    for test in tests:
        success,cause = general_test(test["fn"], test["score"], test["desc"], args.ta)
        if success:
	    your_score += test["score"]
	else:
	    fail_tests.append((test['desc'][:9],cause)) 
