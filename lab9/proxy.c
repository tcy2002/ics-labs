...
/* macros for error-handling (only add printf during debugging) */
...
#define ERROR_HANDLER_FULL(msg) Close(serverfd); ERROR_HANDLER((msg))
    ...
    while ((n = Rio_readlineb(&rio_clt, buf, MAXLINE)) <= 0) {
    ...
    n = strlen(buf);
    if (Rio_writen_w(serverfd, buf, n) <= 0) {
        ERROR_HANDLER_FULL("forward request line error\n");
    }

    /* parse and forward request header */
    body_n = 0;
    do {
        if ((n = Rio_readlineb_w(&rio_clt, buf, MAXLINE)) <= 0) {
            ERROR_HANDLER_FULL("read request header error\n");
        }
        if (Rio_writen_w(serverfd, buf, n) <= 0) {
            ERROR_HANDLER_FULL("forward request header error\n");
        }
    ...
    if (body_n > 0) {
        if ((n = Rio_readnb_w(&rio_clt, buf, body_n)) <= 0) {
    ...
    while (body_n > 0) {
        /* read 1 byte only */
        if ((n = Rio_readnb_w(&rio_svr, buf, 1)) <= 0) {
...
int main(int argc, char **argv)
{
    ...
    while (1) {
        /* wait for client */
        vargp = Malloc(sizeof(int) + clientlen);
