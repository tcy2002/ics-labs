/* name: Tong Chuyan  id: 520021910934 */
...
long_t compute_alu(alu_t op, long_t argA, long_t argB)
{
    ...
    switch (op){
        ...
        default: val = (argA & ~argB) | (~argA & argB);
    }
    return val;
}
...
{
    bool_t doit = FALSE;
    int z = GET_ZF(cc), s = GET_SF(cc), o = GET_OF(cc);
    switch (cond){
        case C_YES: doit = TRUE; break;
        case C_LE: doit = (s != o) || (z == 1); break;
        case C_L: doit = (s != o); break;
        case C_E: doit = (z == 1); break;
        case C_NE: doit = (z == 0); break;
        case C_GE: doit = (s == o); break;
        case C_G: doit = (s == o) && (z == 0);
    }
    return doit;
}
...
stat_t nexti(y64sim_t *sim)
{
    ...
    if ((icode > I_NOP && icode < I_JMP) || icode > I_RET){
        if (!get_byte_val(sim->m, next_pc, &codefun)) {
            err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
    ...
    switch (icode) {
      ...
      case I_MRMOVQ: /* 5:0 regB:regA imm */
          val = get_reg_val(sim->r, regB);
          if (!get_long_val(sim->m, val + imm, &val)){
      ...
      case I_CALL: /* 8:x imm */
          ...
          set_reg_val(sim->r, REG_RSP, val - 0x8);
          if (val == 0){
              err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, val - 0x8);
              return STAT_ADR;
          } 
          ...
          break;
      case I_RET: /* 9:0 */
          get_long_val(sim->m, get_reg_val(sim->r, REG_RSP), &imm);
          set_reg_val(sim->r, REG_RSP, get_reg_val(sim->r, REG_RSP) + 0x8);
          sim->pc = imm;
          break;
      case I_PUSHQ: /* A:0 regA:F */
          imm = get_reg_val(sim->r, regA);
          val = get_reg_val(sim->r, REG_RSP);
          set_reg_val(sim->r, REG_RSP, val - 0x8);
          if (val == 0 && regA != REG_RSP){
              err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, val - 0x8);
              return STAT_ADR;
          }
          set_long_val(sim->m, get_reg_val(sim->r, REG_RSP), imm);
          sim->pc = next_pc;
          break;
